 ...
 * @author Chris Tran
 */
public class HomeResetPasswordController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            UserErrorObject errorObject = bean.validateResetPassword();
            if (errorObject != null) {
                request.setAttribute("ERROROBJECT", errorObject);
                url = INVALID;
            } else {
                if (bean.resetPassword(sessionUsername)) {
                    url = SUCCESS;
                }
            }
        } catch (Exception e) {
            log("ERROR at HomeResetPasswordController: " + e.getMessage());
        } finally {
