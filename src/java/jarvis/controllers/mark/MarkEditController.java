...
public class MarkEditController extends HttpServlet {
    ...
    private static final String HOME = "WEB-INF/mark.jsp";

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.setAttribute("ERROR", "Unable to get mark");
        String url = ERROR;
        try {
            MarkBean bean = new MarkBean();
            BeanPopulator populator = new BeanPopulator();
            populator.populateMark(request, bean);
            MarkDTO dto = bean.getByMarkCode();
            request.setAttribute("DTO", dto);
            MarkSearchDTO search = bean.getMarkSearchDTO();
            request.setAttribute("SEARCH", search);
            url = HOME;
        } catch (Exception e) {
            log("ERROR at MarkEditController: " + e.getMessage());
        } finally {
